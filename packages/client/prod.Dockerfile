...
RUN \
  ...
  else echo "Warning: Lockfile not found. It is recommended to commit lockfiles to version control." && echo ; \
  fi
...
RUN \
  ...
  elif [ -f package-lock.json ]; then npm run build -w packages/client; \
  elif [ -f pnpm-lock.yaml ]; then pnpm build; \
  else npm run build -w packages/client; \
  fi
